...
export default function SocialImpactSection({ simpacts }) {
  ...
  // title, titleAr, keyword,  description, descriptionAr, date, images, details, detailsAr
  return (
        <>
          <div onClick={() => {
            setAddForm(!addForm)
              ...
              <form className='flex flex-col items-center justify-center w-full' onSubmit={editForm ? handleEdit : handleAdd} onChange={() => setAlert('')} >
                ...
                <div dir='rtl' className="fRow">
                  <div className="titleAr w-full">
                    <label htmlFor='titleAr'>عنوان الحدث:</label>
                ...
                <div dir='rtl' className="fRow">
                  <div className="descriptionAr w-full">
                    <label htmlFor="descriptionAr">وصف الحدث:</label>
                ...
                <button className='btnForm'>{editForm ? "Edit" : "Add"} Social Impact</button>
              </form>
              {editForm && <div className="btnForm hover:bg-red-500 cursor-pointer" onClick={() => handleDelete()}>Delete Social Impact</div>}
            </>
      ) : (
        <>
                ...
                {simpacts.map((impact, ind) => (
                  <tr className='text-whiteColor cursor-pointer hover:bg-priColor duration-700'
                    onClick={() => {
                      ...
                      settitleAr(impact.titleAr)
                      setkeyword(impact.keyword)
                      setdescription(impact.description)
                      setdescriptionAr(impact.descriptionAr)
                      setdate(impact.date)
                      setimages(impact.images)
                      setdetails(impact.details)
                      setdetailsAr(impact.detailsAr)
                    }}
                    key={ind}
                  >
                    <td>{impact.title}</td>
                    <td>{new Date(impact.date).toLocaleDateString()}</td>
                    <td>{new Date(impact.createdAt).toLocaleDateString()}</td>
                    <td>{new Date(impact.updatedAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {simpacts.length === 0 && (<h2 className='text-xl text-whiteColor font-bold my-4'>No Social Impacts Added Yet</h2>)}
          </div>
        </>
      )}
    </>
