  ...
  return (
    <>
      <div onClick={() => {
        setAddForm(!addForm)
        setEditForm(false)
      }} className={`ShowBtn fixed bottom-5 right-5 p-3 cursor-pointer bg-whiteColor rounded-xl ${addForm ? "text-green-500" : "text-bgColor"}`}>
      ...
      {addForm ? (
        <>
          ...
          {editForm && (<button onClick={() => DeleteProduct()} className="btnDelete">Delete Blog</button>)}
