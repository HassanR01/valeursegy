        ...
        return (
            <>
                <div className="blogsList w-full flex flex-wrap items-center justify-around">
                    {blogs.map((blog, ind) => (
                        <TransitionLink href={`/blogs/${blog.title}`} className="blog cursor-pointer hover:shadow-xl duration-500 relative w-full lg:w-[45%] min-w-[300px] h-[320px] rounded-tr-xl rounded-bl-xl overflow-hidden m-4" key={ind}>
                            ...
                            <div className="details absolute w-full h-full top-0 left-0 bg-whiteColor bg-opacity-70 flex items-center justify-center flex-col p-4">
                                ...
                                <span className='absolute bottom-2 left-4 text-mainColor text-lg'>{lang === 'ar' ? `اخر تحديث: ${Date(blog.updatedAt)}` : `Last Upadate: ${new Date(blog.updatedAt).toLocaleDateString()}`}</span>
                            </div>
                        </TransitionLink>
                    ))}
                </div>
